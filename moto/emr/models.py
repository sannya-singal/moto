...
class FakeCluster(BaseModel):
    ...
    def master_public_dns_name(self):
        ...
        master_instance = [i for i in self.instances if i.instance_group.id == master_group_id]
        # add region into the domain name after the IP (results in something like: ec2-184-0-0-1.us-west-1.compute.amazonaws.com)
        parts = master_instance[0].details.public_dns.split(".") # there should always be at least one master instance available here
